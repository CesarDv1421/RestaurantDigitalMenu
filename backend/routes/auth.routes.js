...
auth.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ id: userExist.id }, process.env.SECRET_JWT, { expiresIn: '6h' });

      return res.status(201).json({ token, userName: userExist.nombre, rol: userExist.roles.roles });
    } catch (err) {
